...
function transitionSlides() {
  ...
  setTimeout(function() {
    if (cur === bannerBoxContent.length) {
      ...
      bannerBox.style.transition = "0s";
      bannerBox.style.transform = `translateX(${-1400 * (bannerBoxContent.length - 1)}px)`;
      bannerText.style.transition = "0s";
      bannerText.style.transform = `translateX(${-330 * (bannerBoxContent.length - 1)}px)`;
...
let loopInterval = setInterval(() => {
  if (!isPaused) {     // 일시정지 상태가 아닐 때만 슬라이드 전환 수행
    nextMove();
  }
}, 3000);
...
function mainMove() {
  ...
  setTimeout(function () {
    if (mainCurr === mainSlideContent.length - 1) {
      ...
      mainSlide.style.transform = `translateX(${-412 * (mainSlideContent.length - 1)}px)`;
      mainSlide.style.transition = ".5s";
